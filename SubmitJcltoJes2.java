	 ...
	 */
	 ...
	 public void setJcldataset(String pds, String member) throws Exception{
		...
		public void checkJobStatus() throws Exception {
			...
			conf = new FTPClientConfig("com.ca.jesftp");
			ftp.configure(conf);
...
//				FTPFile[] result = ftp.listFiles("*");
//				  for (int i = 0; i < result.length; i++) {
//					  if ( (result[i].getRawListing()).contains(jobNumber))
//					  { 
//						  parseJoblog(result[i].getRawListing());
//					  	System.out.println(result[i].getRawListing());
//					  }
//				}//
				FTPFile[] result = ftp.listFiles("*");
				  for (int i = 0; i < result.length; i++) {
					JesJob job = (JesJob) result[i];
						if (jobNumber.equals(job.getName()))
						{	
